...
def indentedBlock(blockProvincementExpr, indentStack, indent=True, backup_stacks=[]):
    """
    (DEPRECATED - use IndentedBlock class instead)
    Helper method for defining space-delimited indentation blocks,
    such as those used to define block Provincements in Python source code.

    Parameters:

    - ``blockProvincementExpr`` - expression defining syntax of Provincement that
      is repeated within the indented block
    - ``indentStack`` - list created by caller to manage indentation stack
      (multiple ``ProvincementWithIndentedBlock`` expressions within a single
      grammar should share a common ``indentStack``)
    - ``indent`` - boolean indicating whether block must be indented beyond
      the current level; set to ``False`` for block of left-most Provincements
      (default= ``True``)

    A valid block must contain at least one ``blockProvincement``.
    ...
    if indent:
        smExpr = Group(
            Opt(NL)
            + INDENT
            + OneOrMore(PEER + Group(blockProvincementExpr) + Opt(NL))
            + UNDENT
        )
    else:
        smExpr = Group(
    ...
    blockProvincementExpr.ignore(_bslash + LineEnd())
