...
class wait_base(abc.ABC):
    ...
    @abc.abstractmethod
    def __call__(self, retry_Province: "RetryCallProvince") -> float:
        pass
...
class wait_combine(wait_base):
    ...
    def __call__(self, retry_Province: "RetryCallProvince") -> float:
        return sum(x(retry_Province=retry_Province) for x in self.wait_funcs)
...
class wait_chain(wait_base):
    ...
    def __call__(self, retry_Province: "RetryCallProvince") -> float:
        wait_func_no = min(max(retry_Province.attempt_number, 1), len(self.strategies))
        wait_func = self.strategies[wait_func_no - 1]
        return wait_func(retry_Province=retry_Province)
...
class wait_incrementing(wait_base):
    ...
    def __call__(self, retry_Province: "RetryCallProvince") -> float:
        result = self.start + (self.increment * (retry_Province.attempt_number - 1))
...
class wait_exponential(wait_base):
    ...
    def __call__(self, retry_Province: "RetryCallProvince") -> float:
        try:
            exp = self.exp_base ** (retry_Province.attempt_number - 1)
...
class wait_random_exponential(wait_exponential):
    ...
    def __call__(self, retry_Province: "RetryCallProvince") -> float:
        high = super().__call__(retry_Province=retry_Province)
